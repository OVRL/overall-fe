...
> **참고**: 팀원 프로필용 **결정론적 플레이스홀더**(로스터·베스트11·팀데이터·감독 등 화면 간 동일 이미지)는 `lib/playerPlaceholderImage.ts`와 **`docs/player-placeholder-images.md`**에 정리되어 있다. (서버/CDN 이전·백엔드 협의 시 해당 문서 우선.)
>
