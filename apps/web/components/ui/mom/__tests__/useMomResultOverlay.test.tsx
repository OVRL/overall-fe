...
describe("useMomResultOverlay", () => {
  ...
  it("빈 배열로 open 하면 스토어가 바뀌지 않는다", () => {
    ...
    act(() => {
      result.current.openWithCandidates([]);
    });
  ...
  it("open 후 isOpen이 true이고 close 후 false가 된다", () => {
    ...
    act(() => {
      result.current.openWithCandidates(sampleCandidates);
    });
  ...
  it("openByMatch 호출 시 request가 설정된다", () => {
    const { result } = renderHook(() => useMomResultOverlay());
    act(() => {
      result.current.openByMatch({ matchId: 10, teamId: 20 });
    });
    expect(useMomResultOverlayStore.getState().request).toEqual({
      matchId: 10,
      teamId: 20,
    });
  });
