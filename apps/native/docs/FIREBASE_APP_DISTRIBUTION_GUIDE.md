...
- **자동**: **`main` 또는 `develop`** 브랜치에 **`apps/native/`** 아래 코드가 바뀌어 푸시되면 실행됩니다. (같은 푸시에 **iOS·Android 잡이 병렬**로 돕니다.) 문서만 바꾼 경우는 `paths-ignore`로 제외되어 있을 수 있습니다.  
- **수동**: **Actions → “EAS Build → Firebase App Distribution” → Run workflow** 에서 `platform`(ios / android / both), 그룹 이름, 릴리즈 노트를 골라 실행할 수 있습니다.

어느 쪽이든 EAS에서 **빌드가 끝날 때까지 기다린 뒤** 산출물을 받아 **Firebase App Distribution**에 올립니다.
...
### 11.1.1 릴리즈 노트(출시 노트) 넣기

- **수동 실행(workflow_dispatch)**: Run workflow에서 `release_notes` 입력값이 그대로 Firebase 릴리즈 노트로 들어갑니다.
- **자동 실행(push)**: 워크플로가 **이번 푸시에 포함된 커밋 메시지들**을 모아서 릴리즈 노트로 넣습니다. (최대 40줄 제한)
