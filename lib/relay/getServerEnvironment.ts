 ...
 * - accessToken을 넘겨 서버 fetch에서 백엔드 인증을 적용합니다.
 * - isServer: true로 설정해 서버 렌더링 모드로 동작합니다.
 */
export function getServerEnvironment(accessToken: string | null): Environment {
  const source = new RecordSource();
  const store = new Store(source);
  const fetchFn = createServerFetch(accessToken);
