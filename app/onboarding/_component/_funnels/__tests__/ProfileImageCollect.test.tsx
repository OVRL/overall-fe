...
jest.mock("@/components/ImageUploader", () => {
  return ({ onFileSelect }: { onFileSelect: (file: File) => void }) => (
    <div data-testid="mock-uploader">
      <button 
        onClick={() => onFileSelect(new File(["(⌐□_□)"], "profile.png", { type: "image/png" }))}
      >
        Upload File
      </button>
    </div>
...
describe("ProfileImageCollect", () => {
  ...
  it("기본 렌더링 확인", () => {
    ...
    expect(screen.getByText(/프로필 이미지를 등록해주세요/)).toBeInTheDocument();
    expect(screen.getByTestId("mock-uploader")).toBeInTheDocument();
    
    // Button initially disabled (no image)
  ...
  it("이미지 업로드 시 다음 버튼 활성화 및 진행", () => {
    ...
    expect(updateFn({})).toEqual({ profileImage: "blob:http://localhost/mock-url" });
