...
export default function FormationPlayerList({
  ...
  const renderPlayerRow = (player: Player) => {
    ...
    return (
      <div
        ...
        className={`flex items-center p-2 rounded-lg border-2 transition-all cursor-pointer select-none relative mb-2 group ${
          isSelected
      ...
      >
        ...
        {!isBench && currentTeam && (
          <div
            className={`absolute left-0 top-0 w-1 h-full rounded-l-lg opacity-80 
        ...
        {/* Position Badge */}
        <div
          className={`w-8 h-8 rounded flex items-center justify-center mr-3 font-bold text-xs ${
            isBench ? "bg-gray-700 text-gray-400" : "bg-blue-900 text-blue-300"
          }`}
        >
          {player.position}
        </div>
        ...
        <div className="flex-1 min-w-0 flex items-center justify-between pr-2">
          <div
            className={`font-bold text-sm truncate ${isBench ? "text-gray-300" : "text-gray-500"}`}
          >
            {player.name}
          </div>
        </div>
        ...
        <div
          className={`px-2 py-1 rounded text-xs font-bold shadow-sm ${
            (player.quarterCount || 0) === 0
              ? "bg-gray-700 text-gray-500"
              : (player.quarterCount || 0) === 1
                ? "bg-yellow-500 text-black"
                : (player.quarterCount || 0) === 2
                  ? "bg-green-600 text-white"
                  : (player.quarterCount || 0) === 3
  ...
  return (
    <div className="bg-surface-card rounded-xl overflow-hidden h-full flex flex-col border border-border-card shadow-[0_4px_8px_0_rgba(0,0,0,0.5)]">
      {/* Header */}
      <div
        className={`p-4 border-b transition-colors bg-surface-secondary border-gray-700`}
      >
