...
const TeamInfo = ({ name, logo, logoColor, reverse = false, mobileLayout = false }: TeamInfoProps) => {
    return (
        <div className={`flex items-center gap-1.5 lg:gap-4 ${reverse ? "flex-row-reverse" : "flex-row"}`}>
            {/* 로고 */}
            <div className={`${mobileLayout ? "w-6 h-6" : "w-10 h-10"} ${logoColor} rounded-full relative overflow-hidden flex-shrink-0`}>
                <Image
                    src={logo}
                    alt={name}
                    fill
            ...
            {/* 이름 */}
            <span className={`text-white font-medium ${mobileLayout ? "text-[11px] xs:text-sm" : "text-base"} whitespace-nowrap`}>
                {name}
            </span>
        </div>
    );
